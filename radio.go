...
func (r *Radio) Draw(screen tcell.Screen) {
	...
	if r.labelWidth > 0 {
		...
		_, drawnWidth, _ := printWithStyle(screen, r.label, x, y, 0, width, AlignLeft, labelStyle, labelBg == tcell.ColorDefault)
		x += drawnWidth
	}
...
// SetDisabled sets whether or not the radio is disabled. Disabled radios
// cannot be changed.
//
// If the radio is part of a form, you should set focus to the form itself
// after calling this function to set focus to the next non-disabled form item.
func (r *Radio) SetDisabled(disabled bool) FormItem {
	r.disabled = disabled
	return r
}

// GetDisabled returns whether or not the radio is disabled. Disabled radios
// cannot be changed.
//
// If the radio is part of a form, you should set focus to the form itself
// after calling this function to set focus to the next non-disabled form item.
func (r *Radio) GetDisabled() bool {
	return r.disabled
}
